...
public class UserServiceImpl implements UserService{
	 ...
	 */
	@Override
	public int login(String userId,String password) throws Exception,IdNotFoundException,PasswordMismatchException {
		int result=1;
		//1.아이디 존재여부
		User user = userDao.findUser(userId);
		if(user==null) {
			//아이디존재안함
			throw new IdNotFoundException(userId+"는 존재하지 않는 아이디입니다.");
		}
